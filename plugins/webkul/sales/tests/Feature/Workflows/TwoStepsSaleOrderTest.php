...
it('creates the pick leg from stock to output linked to the order line on confirm', function () {
    ...
    expect($move->sale_order_line_id)->toBe($order->lines->first()->id)
        ...
        ->and($move->destination_location_id)->toBe($this->warehouse->output_stock_location_id);
});

it('does not update delivered quantity until the customer-facing ship leg is validated', function () {
    $order = SaleHelper::confirmedOrder($this->warehouse, $this->product, 10);
...
it('links both the pick and ship legs to the sale order once the chain completes', function () {
    ...
    expect($order->operations->filter(fn ($op) => $op->state === OperationState::DONE))->toHaveCount(2)
        ->and(SaleHelper::customerDelivery($order)->moves->first()->destinationLocation->type)->toBe(LocationType::CUSTOMER);
});

it('updates delivered quantity and empties stock after the full two-step chain', function () {
    $order = SaleHelper::confirmedOrder($this->warehouse, $this->product, 10);
