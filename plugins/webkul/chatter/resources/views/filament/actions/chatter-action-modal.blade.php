@php
    ...
    $headerActions = method_exists($action, 'getHeaderActions') ? $action->getHeaderActions() : [];
@endphp
            ...
            <div class="flex items-center gap-4 flex-1">
                @if ($actionModalIcon)
                    <div class="fi-modal-icon-ctn">
                        <div class="fi-modal-icon-bg">
                            {{ \Filament\Support\generate_icon_html($actionModalIcon, size: \Filament\Support\Enums\IconSize::Large) }}
                        </div>
                    </div>
                @endif
            ...
            @if (count($headerActions) > 0)
                <div class="flex items-center gap-2">
                    @foreach ($headerActions as $headerAction)
                        <x-filament::button
                            type="button"
                            wire:click="mountAction('{{ $headerAction->getName() }}')"
                            :color="$headerAction->getColor()"
                            :size="$headerAction->getSize()"
                            :icon="$headerAction->getIcon()">
