...
>
    @if ($icon && $iconPosition === IconPosition::Before)
        {{ \Filament\Support\generate_icon_html($icon, null, new \Illuminate\View\ComponentAttributeBag(['x-show' => '! loading'])) }}
        <span x-show="loading" style="display: none">
            <x-filament::loading-indicator class="h-5 w-5" />
        </span>
    @elseif ($iconPosition === IconPosition::Before || ! $iconPosition)
        <span x-show="loading" style="display: none">
            <x-filament::loading-indicator class="h-5 w-5" />
        </span>
    @endif
